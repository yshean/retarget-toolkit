...
z_range = [(patch_size(3)-1)/2+1 : new_t_size(3)-(patch_size(3)-1)/2];
...
old2new_centers(:,3) = max(old2new_centers(:,3),(patch_size(3)-1)/2+1);
old2new_centers(:,3) = min(old2new_centers(:,3),new_t_size(3)-(patch_size(3)-1)/2);
new_t_matches = (old2new_centers(:,3)-z_range(1)).*(length(y_range)*length(x_range))+...
                (old2new_centers(:,2)-y_range(1)).*length(x_range)+old2new_centers(:,1)-x_range(1)+1;

x_range = [(patch_size(1)-1)/2+1 : old_s_size(1)-(patch_size(1)-1)/2];
y_range = [(patch_size(2)-1)/2+1 : old_s_size(2)-(patch_size(2)-1)/2];
z_range = [(patch_size(3)-1)/2+1 : old_s_size(3)-(patch_size(3)-1)/2];
...
new2old_centers(:,3) = max(new2old_centers(:,3),(patch_size(3)-1)/2+1);
new2old_centers(:,3) = min(new2old_centers(:,3),old_s_size(3)-(patch_size(3)-1)/2);
new_s_matches = (new2old_centers(:,3)-z_range(1)).*(length(y_range)*length(x_range))+...
                (new2old_centers(:,2)-y_range(1)).*length(x_range)+new2old_centers(:,1)-x_range(1)+1;
